#!/usr/bin/env python3
"""
FUACS Context Merger (JSON Output)
Merges all YAML documentation files into a single comprehensive JSON context file.
"""
...
class ContextMergerJSON:
    ...
    def merge_all(self) -> Dict[str, Any]:
        ...
        print("=" * 60)
        print("FUACS Context Merger (JSON)")
        print("=" * 60)
        ...
        if system_context:
            self.merged_data["fuacs_complete_context"]["system_context"] = (
        ...
        if requirements_data:
            self.merged_data["fuacs_complete_context"]["requirements"] = (
                requirements_data
            )

        return self.merged_data

    def save_merged_context(self, output_path: str = "final_context.json"):
...
def main():
    """Main entry point."""
    try:
        merger = ContextMergerJSON(root_dir=".")
        merger.merge_all()
        success = merger.save_merged_context("final_context.json")
